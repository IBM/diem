...
class App {
    ...
    public start = async (): Promise<void> => {
        ...
        const ass: { [index: string]: { src: string; integrity: string } } = assets;

        env.script = [];

        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        for (const [_key, asset] of Object.entries(ass)) {
            if (asset.src.includes('bundle.')) {
                env.script.push({
                    name: `${env.path}/${asset.src}`,
                    sri: asset.integrity,
                });
            }

            if (asset.src.includes('.css')) {
                env.css.push(`${env.path}/${asset.src}`);
            }
        }
