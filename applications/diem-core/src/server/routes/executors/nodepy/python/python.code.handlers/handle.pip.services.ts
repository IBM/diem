...
const py_pipinstall: (id: string, pip: string[]) => string = (
    _id: string,
    pip: string[]
): string => String.raw`### handle.pip.services (py_pipinstall) ###
...
export const handlePipServices: (code: string, doc: IJobSchema) => Promise<string> = async (
    code: string,
    doc: IJobSchema
): Promise<string> => {
    try {
